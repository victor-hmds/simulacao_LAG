...
# 1 Piu peso 0,419
    ...
    #y1 = 2.5751 * np.exp(-p/0.3375) + 0.9879
    #print ('Valor excitado na curva de potencial Piu 1', y1)

#count, bins, ignored = plt.hist(y1, 100, density=True, histtype = 'stepfilled', facecolor = 'green')
#plt.plot(bins, 1/(sigma * np.sqrt(2 * np.pi)) * np.exp( - (bins - mu)**2 / (2 * sigma**2) ), linewidth=2, color='red')
#plt.savefig('Distribuição de Energia Cinética.png')                                      # Salvando imagem
#plt.title('Distribuição de Energia Cinética')                                            # Definindo título
#plt.xlabel('Energia')                                                # Definindo nome do eixo X
